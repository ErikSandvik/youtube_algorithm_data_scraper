...
def plot_overall_exposure(metrics: Dict, output_dir: Path):
    """
    Create a simple bar chart showing overall political exposure.
    ...
    # Plot 1: Political vs Non-Political
    ...
    # Plot 2: Political Content Depth
    ...
    bars2 = ax2.bar(depth_labels, depth_values, color='#e74c3c', alpha=0.7, edgecolor='black')

    # Add value labels
    for bar, val in zip(bars2, depth_values):
        height = bar.get_height()
...
def plot_exposure_by_iteration(df_iteration: pd.DataFrame, output_dir: Path):
    """
    Plot political exposure rate across iterations (crawl depth).

    Args:
        df_iteration: DataFrame with iteration-level metrics
        output_dir: Directory to save figure
    """
    fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(14, 5))

    # Plot 1: Political percentage by iteration
    ...
    # Plot 2: Total recommendations by iteration
...
def plot_exposure_by_position(df_position: pd.DataFrame, output_dir: Path):
    """
    Plot political exposure by recommendation position/rank.

    Args:
        df_position: DataFrame with position-level metrics
        output_dir: Directory to save figure
    """
    fig, ax = plt.subplots(figsize=(12, 6))

    # Create bar plot
    bars = ax.bar(df_position['position'], df_position['political_percentage'],
...
def plot_top_videos(df_videos: pd.DataFrame, output_dir: Path, top_n: int = 15):
    """
    Plot top political videos by recommendation frequency.

    Args:
        df_videos: DataFrame with video-level metrics
        output_dir: Directory to save figure
        top_n: Number of videos to show
    """
    ...
    bars = ax.barh(range(len(df_plot)), df_plot['recommendation_count'],
                   color='#e74c3c', alpha=0.7, edgecolor='black')
...
def plot_top_channels(df_channels: pd.DataFrame, output_dir: Path, top_n: int = 15):
    """
    Plot top channels by political content recommendations.

    Args:
        df_channels: DataFrame with channel-level metrics
        output_dir: Directory to save figure
        top_n: Number of channels to show
    """
    fig, ax = plt.subplots(figsize=(12, 8))
    ...
    # Create horizontal bar chart
    bars = ax.barh(range(len(df_plot)), df_plot['total_recommendations'],
...
def plot_run_comparison(df_runs: pd.DataFrame, output_dir: Path):
    """
    Compare political exposure across different crawl runs.

    Args:
        df_runs: DataFrame with run-level metrics
        output_dir: Directory to save figure
    """
    if len(df_runs) < 2:
    ...
    bars = ax.bar(x_pos, df_runs['political_percentage'],
                  color='#e74c3c', alpha=0.7, edgecolor='black')
...
def export_tables(summary: Dict, output_dir: Path):
    """
    Export metric tables to CSV files for inclusion in papers.

    Args:
        summary: Dictionary with all metrics
        output_dir: Directory to save tables
    """
    # Export iteration metrics
    ...
    # Export overall metrics as a simple table
...
def generate_all_visualizations(summary: Dict, output_dir: str = "analysis/outputs"):
    """
    Generate all figures and export all tables.
